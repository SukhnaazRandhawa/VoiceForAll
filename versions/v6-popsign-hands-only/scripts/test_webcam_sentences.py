...
# #     # Create word list with confidence indicators
# #     words_with_conf = []
# #     for word, conf in zip(words, confidences):
# #         if conf >= 0.70:
# #             words_with_conf.append(f"{word} (confident)")
# #         elif conf >= 0.50:
# #             words_with_conf.append(f"{word} (medium)")
# #         else:
# #             words_with_conf.append(f"{word} (uncertain)")
    
# #     prompt = f"""You are a sign language translator. Convert these words into ONE natural English sentence.

# # Words with confidence levels:
# # {words_with_conf}

# # RULES:
# # 1. Words marked "confident" - ALWAYS use them
# # 2. Words marked "medium" - Use them if they fit the context
# # 3. Words marked "uncertain" - ONLY use if they fit naturally with other words. If they don't make sense with the other words, IGNORE them completely.
# # 4. Add grammar words (I, want, to, the, a, is, am, etc.) to make it natural
# # 5. Output ONLY the final sentence. No explanations.

# # Example 1:
# # Words: ["chair (confident)", "sit (confident)", "apple (confident)", "crocodile (uncertain)"]
# # Output: I want to sit on the chair and eat an apple.
# # (crocodile ignored - doesn't fit context)

# # Example 2:
# # Words: ["dog (confident)", "bad (uncertain)", "bath (confident)"]
# # Output: The bad dog needs a bath.
# # (bad kept - fits context with dog)

# # Example 3:
# # Words: ["bed (confident)", "sleep (confident)", "airplane (uncertain)"]
# # Output: I want to sleep on the bed.
# # (airplane ignored - doesn't fit context)

# # Now convert these words:
...
#     # Create word list with confidence levels (for internal use only)
#     high_conf_words = []
#     medium_conf_words = []
#     low_conf_words = []
...
# HIGH CONFIDENCE WORDS (definitely use these): {high_conf_words if high_conf_words else "none"}
# MEDIUM CONFIDENCE WORDS (use if they fit): {medium_conf_words if medium_conf_words else "none"}
# LOW CONFIDENCE WORDS (only use if they fit naturally, otherwise ignore): {low_conf_words if low_conf_words else "none"}

# RULES:
# 1. Always use the HIGH CONFIDENCE words
# 2. Use MEDIUM and LOW confidence words only if they make sense in the sentence
# 3. If a LOW confidence word doesn't fit with the other words, completely ignore it
# 4. Add grammar words (I, want, to, the, a, is, am, etc.) to make it natural
# 5. Output ONLY the final sentence - no explanations, no labels, no brackets

# Example 1:
# HIGH: [chair, sit]
# LOW: [crocodile]
# Output: I want to sit on the chair.
...
# HIGH CONFIDENCE: {high_conf_words if high_conf_words else "none"}
# MEDIUM CONFIDENCE: {medium_conf_words if medium_conf_words else "none"}
# LOW CONFIDENCE: {low_conf_words if low_conf_words else "none"}
...
#             # Remove any accidental confidence labels that might leak through
#             sentence = sentence.replace("(confident)", "")
#             sentence = sentence.replace("(medium)", "")
#             sentence = sentence.replace("(uncertain)", "")
#             sentence = sentence.replace("confident", "")  # Remove if it appears
